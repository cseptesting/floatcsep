...
def truncated_GR(bval, mag_bin, mmin, mmax, n=1):
    ...
    mag_weights = (10 ** (aval - bval * (mag_bins - mag_bin / 2.)) - 10 ** (
                aval - bval * (mag_bins + mag_bin / 2.))) / \
                  (10 ** (aval - bval * (mmin - mag_bin / 2.)) - 10 ** (
                              aval - bval * (mmax + mag_bin / 2.)))
    return aval, mag_bins, numpy.array(mag_weights)


def make(raw, forecast_path, N=None, bval=None, mmin=5.0, mmax=8.0):
    _, mws, mw_weights = truncated_GR(bval, mag_bin=0.1, mmin=mmin, mmax=mmax,
                                      n=N)
    ...
    if data.columns.shape[0] <= 12:
        ...
        rates = spatial_rate[:, numpy.newaxis] * mw_weights[numpy.newaxis,
                                                 :] * N
        data.drop(['rate', 'mask', 'm_min', 'm_max', 'mask', 'dispersion'],
                  axis=1, inplace=True)  # only poisson
...
def main():
    parser = argparse.ArgumentParser(
        description='Creates New Zealand models for a given yearly rate and b-val')
