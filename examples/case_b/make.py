...
min_mag = 4.0
max_mag = 8.0
magnitudes = cleaner_range(min_mag, max_mag, 0.1)
...
catalog = query_comcat(start_time=start, end_time=end,
                       min_magnitude=min_mag,
                       min_longitude=bbox[0],
                       max_longitude=bbox[1],
                       min_latitude=bbox[2],
                       max_latitude=bbox[3])
for i, j in enumerate(catalog.data):
    origin = origins[int(j[2] > (bbox[2] + bbox[3]) / 2.)] + np.random.random(
        2)
    events.append([i, j[1], origin[1], origin[0], j[4], j[5]])
new_cat = CSEPCatalog(data=events)
new_cat.write_json('catalog.json')
